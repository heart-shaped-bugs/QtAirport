...
void MainWindow::on_btnEditAircraft_clicked()
{
    ...
    if (row >= 0 && row < aircraftList.size())
    {
        // Изменяем данные самолёта
        aircraftList[row].setModel(QString("Изменённая модель"));
        aircraftList[row].setCapacity(200);
        aircraftList[row].setAirline(QString("Изменённая авиакомпания"));

        // Обновляем таблицу
        ui->aircraftTable->setItem(row, 1, new QTableWidgetItem(aircraftList[row].getModel()));
        ui->aircraftTable->setItem(row, 2, new QTableWidgetItem(QString::number(aircraftList[row].getCapacity())));
        ui->aircraftTable->setItem(row, 3, new QTableWidgetItem(aircraftList[row].getAirline()));
    }
}
...
void MainWindow::on_btnLoadAircraft_clicked()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, "Загрузить самолёты", "", "XML файлы (*.xml);;All files (*)");
...
void MainWindow::on_btnDeletePassenger_clicked()
{
    ...
    if (row >= 0 && row < passengerList.size()) {
        ...
        for (int i = 0; i < passengerList.size(); ++i) {
            ...
            ui->passengerTable->setItem(i, 0, new QTableWidgetItem(QString::number(passengerList[i].getId())));
        }
    }
}
...
void MainWindow::on_btnEditPassenger_clicked()
{
    ...
    if (row >= 0 && row < passengerList.size()){
        passengerList[row].setFirstName(QString("Новое имя"));
        passengerList[row].setLastName(QString("Новая фамилия"));
        passengerList[row].setFlightNumber(QString("Новый рейс"));

        ui->passengerTable->setItem(row, 1, new QTableWidgetItem(passengerList[row].getFirstName()));
        ui->passengerTable->setItem(row, 2, new QTableWidgetItem(passengerList[row].getLastName()));
        ui->passengerTable->setItem(row, 3, new QTableWidgetItem(passengerList[row].getFlightNumber()));
    }
}
...
{
    // Открываем диалог открытия файла
    QString fileName = QFileDialog::getOpenFileName(this, "Загрузить пассажиров", "", "XML файлы (*.xml);;All files (*)");
